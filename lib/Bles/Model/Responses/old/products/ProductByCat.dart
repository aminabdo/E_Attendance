...
class ProductByCatResponse extends BaseResponse {
  int status;
  List<DataBean> data;
  String message;

  static ProductByCatResponse fromMap(Map<String, dynamic> map) {
    if (map == null) return null;
    ProductByCatResponse productByCatBean = ProductByCatResponse();
    productByCatBean.status = map['status'];
    productByCatBean.data = List()..addAll(
      (map['data'] as List ?? []).map((o) => DataBean.fromMap(o))
    );
    productByCatBean.message = map['message'];
    return productByCatBean;
  }
