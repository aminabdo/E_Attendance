...
class HomeBloc extends BaseBloc {
  BehaviorSubject<GetMainCatResponse> _get_main_cat =
      BehaviorSubject<GetMainCatResponse>();
  BehaviorSubject<FilterWebSiteResponse> _filter_website =
      BehaviorSubject<FilterWebSiteResponse>();
  BehaviorSubject<SearchByNameResponse> _search_by_name =
      BehaviorSubject<SearchByNameResponse>();
  ...
  get_main_cat() async {
    ...
    GetMainCatResponse response = GetMainCatResponse.fromMap(
        (await repository.get(ApiRoutes.get_main_cat())).data);
  ...
  getProductsByCat(int cat_id) async {
    ...
    FilterWebSiteResponse response = FilterWebSiteResponse.fromMap(
        (await repository.get(ApiRoutes.filter_website(cat_id))).data);
  ...
  search_by_name(String name) async {
    ...
    SearchByNameResponse response = SearchByNameResponse.fromMap(
        (await repository.get(ApiRoutes.search_by_name(name))).data);
  ...
  filter(
      ...
      String to = "20",
      String type = "2",
      String cat_id = "1"}) async {
    _home.value = HomeResponse();
    _home.value.loading = true;
    HomeResponse response = HomeResponse.fromMap(
        (await repository.get(ApiRoutes.filter(from, to, type))).data);
  ...
  getHome() async {
    ...
    _home.value.loading = true;
    HomeResponse response = HomeResponse.fromMap(
        await (await repository.get(ApiRoutes.home())).data);
    _home.value = response;
    _home.value.loading = false;
  }

  BehaviorSubject<GetMainCatResponse> get s_get_main_cat => _get_main_cat;
  BehaviorSubject<FilterWebSiteResponse> get s_filter_website =>
      _filter_website;
